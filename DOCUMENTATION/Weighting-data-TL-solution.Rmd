...
<p style="color:#FF3333";>Yes, that's how I would do it</p>
...
input_demo_wts <- bind_cols(
  ...
  data.frame(the_weights= weights(rake_unweighted_input))
  ) %>% 
  rename(actually_prob  = rake_unweighted_input...prob...) %>% 
  select(ID:clin_status, actually_prob, the_weights) %>% 
  arrange(desc(actually_prob))

tail(input_demo_wts)

filter(input_demo_wts, between(actually_prob, .98, 1.02))
