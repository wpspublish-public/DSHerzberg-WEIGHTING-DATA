...
The remainder of this snippet restructures, relables, and sorts this new data frame `input_demo_wts` containing the case-wise weighting multiplers), by applying `dplyr` functions as described previously. It then cleans up the environment by removing unneeded objects.
```{r weights, echo = 32:42, eval = F}
```

<br>

#### Apply multipliers to input data, save files for downstream analysis
