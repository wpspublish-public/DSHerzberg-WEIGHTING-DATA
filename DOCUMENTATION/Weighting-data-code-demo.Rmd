...
###### COMMENTED SNIPPETS
The `survey` package requires as input the census target counts for the four key demographic variables. We obtain these counts from `census_match_input`, one of the date files read in upstream.

In the next snippet, we use `purrr::map_df()` to iterate over the character vector `var_order_census_match`, which contains the names of the four demographic variables. `map_df()` returns a data frame as output (instead of a list). The code within `map_df()` will be executed once for each element of `var_order_census_match`, and that element is substituted into the code wherever the shorthand `.x` appears.
