...
```
Recall that each possible crossing of the four demographic variables is associated with a unique weighting multiplier. In the current input sample, multiplying out the categories of `gender`, `educ`, `ethnic`, and `region` yields 160 possible crossings. However, in this sample (as is often the case), a few of the possible crossings contain no cases. In calculating weighting multipliers, the current method omits empty crossings.

We can create a lookup table `demo_weight_by_crossing` that provides the weighting multiplier for each crossing that contains cases in the input sample. We can use this table to apply weighting multipliers to any subset or alternate version of the input sample (as long as the file has columns for all four demographic variables, to allow correct alignment of rows with the lookup table).

To build the lookup table, we first `group_by(demo_wt)`, and then use `summarize()` to collapse the table vertically, returning a single row for each unique value of `demo_wt`. By definition, each value of `demo_wt` is associated with only one value on each of the four demographic variables. We can extract these single values with the summary function `first()`, which in this instance returns the first of a series of identical values. `summarize()` recreates the four demographic columns, populated with the values associated with each value of `demo_wt`. We then `arrange()` the rows to `match()` the category order previously specified in the vector `cat_order`.

Each row of the resulting lookup table contains the combination of demographic categories that defines a crossing (e.g., `female X high-school diploma X hispanic X south region`), followed by the demographic weighting multiplier associated with that crossing.
```{r apply-weights, echo = 40:55, eval = F}
```
<br>
