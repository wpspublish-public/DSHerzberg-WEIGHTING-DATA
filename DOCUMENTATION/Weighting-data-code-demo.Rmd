...
<!--First, we write a function for printing well-formatted summary tables, using the `huxtable` package.-->
```{r hux-format, eval = T, echo = F}
...
```
We create a comparison table <!--`cat_count_comp`--> with the column `pct_diff`, which expresses, for each demographic variable and category, the difference between the input sample percentage and the census target percentage.
...
Analogously, we would expect cells whose sample percentages closely approximate the census targets to have multipliers close to 1. Because they are neither under- nor over-sampled, those cells would need little adjustment to the numerical impact of their collective item scores.
...
hux_format(filter(input_demo_wts, between(samp_prob, .98, 1.02)) %>%
             select(-age_range,-clin_status,-(i01:i50),-ratio))
```
We now examine the top (`head`) of the data frame, which contains cases from over-sampled crossings. Here, we expect to see high `samp_prob` and low `demo_wt`.
```{r head, eval = T}
...
```{r plot-samp-prob-demo-wt, eval = T}
...
We find the expected relationship between these two variables. In under-sampled categories (e.g., `educ = No_HS`), where `n_diff` is negative, we expect `sum_diff` to also be negative, because the weighted sum of total scores needs to be greater than the unweighted sum, so that the census-adjusted item scores can have the same numerical impact that they would have had, had the census target been achieved. An analogous relationship is observed in over-sample categories (e.g., `ethnic = white`).

Across categories, we see that the absolute magnitude of `sum_diff` varies as expected with `n_diff`, such that categories with the greatest level of deviance from the census target also have the greatest disparity between weighted and unweighted sums of total scores.

We can plot these relationships separately for each demographic variable.
```{r plot, eval = T}
...
```
The graphs show a near-perfect correlation between the extent of mis-sampling and the amount of numerical to item scores applied by the weighting procedure.
